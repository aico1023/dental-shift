...
const State = {
  ...
  dayClipboard: null,        // コピーした一日シフト (shifts, reception, unitNames)
...
// ---- 自動マイグレーション機能 (日曜始まりから月曜始まり) ----
function migrateToMondayStart() {
  const migrated = localStorage.getItem('dsa_monday_migrated');
  if (migrated === 'true') {
    return; // すでに移行済み
  }

  console.log('Migrating data from Sunday-start to Monday-start...');

  // ヘルパー：月曜始まりの週キーを取得
  const getMondayWeekKey = (dateStr) => {
  ...
  // 1. weekShifts の移行
  if (State.weekShifts && Object.keys(State.weekShifts).length > 0) {
    const newWeekShifts = {};
    Object.keys(State.weekShifts).forEach(oldWk => {
      const days = State.weekShifts[oldWk];
      if (!days) return;
      Object.keys(days).forEach(dk => {
        const newWk = getMondayWeekKey(dk);
        if (!newWeekShifts[newWk]) {
          newWeekShifts[newWk] = {};
        }
        newWeekShifts[newWk][dk] = days[dk];
        if (newWk !== oldWk) {
          hasChanges = true;
        }
      });
    });
    State.weekShifts = newWeekShifts;
  }

  // 2. receptionShifts の移行
  if (State.receptionShifts && Object.keys(State.receptionShifts).length > 0) {
    const newReceptionShifts = {};
    Object.keys(State.receptionShifts).forEach(oldWk => {
      const days = State.receptionShifts[oldWk];
      if (!days) return;
      Object.keys(days).forEach(dk => {
        const newWk = getMondayWeekKey(dk);
        if (!newReceptionShifts[newWk]) {
          newReceptionShifts[newWk] = {};
        }
        newReceptionShifts[newWk][dk] = days[dk];
        if (newWk !== oldWk) {
          hasChanges = true;
        }
      });
    });
    State.receptionShifts = newReceptionShifts;
  }

  // 3. leaveRecords の移行
  if (State.leaveRecords && Object.keys(State.leaveRecords).length > 0) {
    const newLeaveRecords = {};
    Object.keys(State.leaveRecords).forEach(oldWk => {
      const staffRecords = State.leaveRecords[oldWk];
      if (!staffRecords) return;
      Object.keys(staffRecords).forEach(staffId => {
        const days = staffRecords[staffId];
        if (!days) return;
        Object.keys(days).forEach(dk => {
          const newWk = getMondayWeekKey(dk);
          if (!newLeaveRecords[newWk]) {
            newLeaveRecords[newWk] = {};
          }
          if (!newLeaveRecords[newWk][staffId]) {
            newLeaveRecords[newWk][staffId] = {};
          }
          newLeaveRecords[newWk][staffId][dk] = days[dk];
          if (newWk !== oldWk) {
            hasChanges = true;
          }
        });
    ...
    State.leaveRecords = newLeaveRecords;
  }

  // 完了フラグをローカルストレージに保存
  localStorage.setItem('dsa_monday_migrated', 'true');
...
function copyCurrentDay() {
  ...
  const shifts = State.weekShifts[wk] && State.weekShifts[wk][dk] 
    ? JSON.parse(JSON.stringify(State.weekShifts[wk][dk])) 
    : {};
    
  const reception = State.receptionShifts[wk] && State.receptionShifts[wk][dk]
    ? JSON.parse(JSON.stringify(State.receptionShifts[wk][dk]))
    : { am: [], pm: [] };

  const unitNames = State.unitNames[dk]
    ? JSON.parse(JSON.stringify(State.unitNames[dk]))
    : null;

  State.dayClipboard = { shifts, reception, unitNames };
  showToast('success', '一日コピー', 'この日のシフトをコピーしました。別の日に「一日貼り付け」ができます。');
}

function pasteToCurrentDay() {
  if (!State.dayClipboard) {
    showToast('warning', '貼り付け不可', 'コピーされた一日のデータがありません。先に「一日コピー」を行ってください。');
    return;
  }
  ...
  if (holidayName) {
    showToast('warning', '貼り付け不可', `祝日（${holidayName}）には貼り付けできません。`);
    return;
  }

  // 1. シフトの貼り付け
  if (!State.weekShifts[wk]) State.weekShifts[wk] = {};
  State.weekShifts[wk][dk] = JSON.parse(JSON.stringify(State.dayClipboard.shifts));
  
  // シフトIDの再生成
  Object.values(State.weekShifts[wk][dk]).forEach(arr => {
  ...
  // 2. 受付シフトの貼り付け
  if (!State.receptionShifts[wk]) State.receptionShifts[wk] = {};
  State.receptionShifts[wk][dk] = JSON.parse(JSON.stringify(State.dayClipboard.reception));

  // 3. ユニット名の貼り付け
  if (State.dayClipboard.unitNames) {
    State.unitNames[dk] = JSON.parse(JSON.stringify(State.dayClipboard.unitNames));
  } else {
  ...
  showToast('success', '一日貼り付け', 'この日のシフトを上書き貼り付けしました。');
}

// ---- シフト管理（期間指定一括登録） ----
function addBulkShifts(staffId, unitNum, startDateStr, endDateStr, startTimeStr, endTimeStr, skipHolidays, selectedDays = null, triggerSave = true) {
  ...
  const startSlot = timeToSlot(sh, sm);
  const endSlot = timeToSlot(eh, em);
  if (isNaN(startSlot) || isNaN(endSlot) || startSlot >= endSlot) {
    return { success: false, message: '時刻の指定が不正です。' };
  }
  ...
  while (currentD <= endD) {
    const dk = dayKeyFromDate(currentD);
    const dayOfWeek = currentD.getDay(); // 0=日, 1=月...

    // 曜日指定のフィルタリング (selectedDays: [sun, mon, tue, wed, thu, fri, sat])
    if (selectedDays && !selectedDays[dayOfWeek]) {
      currentD.setDate(currentD.getDate() + 1);
      continue;
    }

    if (skipHolidays) {
      const isHolidayOrCustom = getHolidayName(dk);
      const isSunday = (dayOfWeek === 0);
      if (isHolidayOrCustom || isSunday) {
        skippedDaysCount++;
        currentD.setDate(currentD.getDate() + 1);
        continue;
      }
    ...
    const wk = weekKeyFromDate(getWeekStart(currentD));
    const existingArr = getUnitShifts(wk, dk, unitNum);

    let isAlreadyScheduled = false;
    for (let u = 1; u <= TOTAL_UNITS; u++) {
      const arr = getUnitShifts(wk, dk, u);
      const overlappingShift = arr.find(sh => {
        const staffIds = [
          sh.doctorId,
          ...(sh.daIds || []),
          ...(sh.dhIds || []),
        ...
        return staffIds.includes(staffId) && slotsOverlap(sh.startSlot, sh.endSlot, startSlot, endSlot);
      });
      if (overlappingShift) {
    ...
    if (staff.role === 'dr') {
      const existingDrShift = existingArr.find(sh =>
      ...
      if (drShift) {
        ...
        if (dhShift.daIds.length < 2 && !dhShift.daIds.includes(staffId)) {
  ...
  if (triggerSave && addedDaysCount > 0) {
    saveAll();
  }
  ...
  return {
    ...
    addedDaysCount,
    skippedDaysCount,
    message: `${addedDaysCount}日分の一括登録が完了しました。${skippedDaysCount}日分はスキップまたは重複のため除外されました。`
  };
}

// ---- 複数スタッフ・週間グリッド一括シフト登録 ----
function addBulkShiftsGroup(configs, startDateStr, endDateStr, startTimeStr, endTimeStr, skipHolidays) {
  let totalAdded = 0;
  let totalSkipped = 0;
  let successCount = 0;

  configs.forEach(cfg => {
    const res = addBulkShifts(
      cfg.staffId,
      cfg.unitNum,
      startDateStr,
      endDateStr,
      ...
      skipHolidays,
      cfg.selectedDays,
      false // triggerSave = false (一括処理中は保存を遅延)
    );
    if (res.success) {
      totalAdded += res.addedDaysCount;
      totalSkipped += res.skippedDaysCount;
      if (res.addedDaysCount > 0) {
        successCount++;
      }
    }
  });
  ...
  return {
    ...
    totalSkipped,
    successCount,
    message: `${successCount}名のスタッフについて、計${totalAdded}日分のシフトを一括登録しました。`
  };
}

// 指定された日にスタッフがシフト（ユニット配置または受付）に入っているか判定するヘルパー関数
function isStaffAssignedOnDay(staffId, weekKey, dayKey) {
  const targetStaffIdStr = String(staffId);
  ...
  for (let u = 1; u <= TOTAL_UNITS; u++) {
    ...
    for (const sh of arr) {
      ...
      const doctors = sh.doctorId ? [String(sh.doctorId)] : [];
      const dhs = Array.isArray(sh.dhIds) ? sh.dhIds.map(String) : [];
      const das = Array.isArray(sh.daIds) ? sh.daIds.map(String) : [];
      const dts = Array.isArray(sh.dtIds) ? sh.dtIds.map(String) : [];
      const tcs = Array.isArray(sh.tcIds) ? sh.tcIds.map(String) : [];
      
      const allStaffInShift = [...doctors, ...dhs, ...das, ...dts, ...tcs];
      if (allStaffInShift.includes(targetStaffIdStr)) {
        return true;
      }
    }
  }
  ...
  if (rc) {
    const amList = Array.isArray(rc.am) ? rc.am.map(String) : [];
    const pmList = Array.isArray(rc.pm) ? rc.pm.map(String) : [];
    if (amList.includes(targetStaffIdStr) || pmList.includes(targetStaffIdStr)) {
      return true;
    }
  }
...
// ---- シフト管理（出勤・お休み一括設定） ----
function setBulkLeaves(staffId, startDateStr, endDateStr, selectedDays, leaveType, triggerSave = true) {
  const staff = getStaff(staffId);
  if (!staff) return { success: false, message: 'スタッフが見つかりません。' };
  ...
  while (currentD <= endD) {
    ...
    // シフトで「お休み（shift-off / normal-leave）」に設定しようとしている場合、
    ...
    setLeaveRecord(wk, staffId, dk, leaveType);
    count++;
...
// アサイン保護により特定の日にシフトが残っていても、他の非配置日の設定が正しく反映されるよう、
// シフトに入っていない日の設定状態を優先して判定する
function getBulkDayCheckboxesState(staffId, startDateStr, endDateStr) {
  const startD = parseLocalDate(startDateStr);
  const endD = parseLocalDate(endDateStr);
  const states = Array(7).fill(true); // デフォルトすべて出勤(true)
  ...
  const currentD = new Date(startD);
  while (currentD <= endD) {
    daysByWeekDay[currentD.getDay()].push(new Date(currentD));
    currentD.setDate(currentD.getDate() + 1);
  }
